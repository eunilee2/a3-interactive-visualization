...
// persist current x-domain when brush zooms
let currentXDomain: [Date, Date] | null = null;
	...
	function draw() {
		...
		// create a unique clipPath so plotted elements don't overflow when zoomed
		...
		const fullXExtent = d3.extent(series, d => d.date) as [Date, Date];
		// clamp currentXDomain to be inside the full extent if present
		if (currentXDomain) {
	...
	const y = d3.scaleLinear().domain([0, yDomainMax]).range([innerHeight, 0]).nice();
		...
		// prepare brush variables (brush created after plotting so it sits on top)
		let brush: any;
		let brushG: any;

		function brushed(event: any) {
			if (!event.selection) return; // nothing selected
		...
		// double-click on SVG to reset zoom
		svg.on('dblclick', () => {
			currentXDomain = null;
			draw();
		});
		...
		// create brush overlay on top so it receives pointer events
